...
and be free for all use. Liquibase (FSL-1.1) is retained for Java apps — see
...
**Licensing note**: Liquibase v5+ uses FSL-1.1 (not OSI-approved). The FSL non-compete clause
...
### 4. demo-be-golang-gin: **goose** (pressly/goose)
...
| Tool                    | Stars    | License    | Pros                                       | Cons                                                   |
| ----------------------- | -------- | ---------- | ------------------------------------------ | ------------------------------------------------------ |
| **Liquibase**           | ~11k     | FSL-1.1    | Richer rollback; consistent with Java apps | Ktor community does not use it; adds XML/YAML overhead |
| **Exposed SchemaUtils** | Built-in | Apache 2.0 | Zero setup                                 | Only creates; cannot add/rename/alter/drop columns     |

### 6. demo-be-fsharp-giraffe: **DbUp**
...
### 7. demo-be-clojure-pedestal: **Migratus**
...
### 8. demo-be-ts-effect: **@effect/sql Migrator** (built-in)
...
### 9. demo-be-csharp-aspnetcore: **EF Core Migrations** (upgrade)
...
3. **Migrations run on startup**: Each app runs pending migrations during application startup (dev
   and production). No separate `migrate` Nx target needed — this matches the existing pattern.
...
6. **All tools must be OSI-licensed**: No FSL, SSPL, BSL, or other non-OSI licenses.
...
| `docs/explanation/software-engineering/licensing/` (new) | Create licensing decisions doc documenting: Liquibase FSL-1.1 decision (why kept, non-compete does not apply), LGPL justifications (Hibernate, sharp-libvips), dual-license elections (Logback → EPL-1.0), and quarterly audit schedule |

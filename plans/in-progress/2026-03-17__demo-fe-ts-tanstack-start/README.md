...
2. Mark auth-dependent components with `"use client"` equivalent (TanStack Start uses
   `createClientComponent` or `useEffect`-based hydration guard)
...
**Note**: The `#__next-route-announcer__` filter in some E2E steps is Next.js-specific.
TanStack Start uses a different route announcer ID or none. The E2E steps use `.filter()`
with `hasNot` which tolerates absence — no DOM changes needed.
...
`.output/server/index.mjs`. The `BACKEND_URL` build arg is baked into the server bundle
as an environment variable read at startup.
...
- Service `demo-be-db`: PostgreSQL 17, container `demo-fe-tss-db`, volume `demo-fe-tss-db-data`
...
Scenario: Mobile layout shows hamburger
...
### Phase 8: Docker, Infrastructure, and CI

- [ ] Step 8.1: Create `apps/demo-fe-ts-tanstack-start/Dockerfile` using multi-stage
      build:
  - Stage `deps`: `node:24-alpine`, install from `package.json`/`package-lock.json`
  - Stage `build`: copy deps, copy source, set `ARG BACKEND_URL`, run `npm run build`
  - Stage runtime: `node:24-alpine`, non-root `app` user, copy `.output/`, `EXPOSE 3301`,
    `CMD ["node", "server/index.mjs"]`
- [ ] Step 8.2: Create `infra/dev/demo-fe-ts-tanstack-start/docker-compose.yml`:
...
- [ ] Step 8.3: Create `.github/workflows/test-demo-fe-ts-tanstack-start.yml`:
...
```gherkin
Scenario: Phase 8 Docker build succeeds
...
### Phase 9: Unit Tests and Coverage

- [ ] Step 9.1: Create `src/test/setup.ts` — Vitest setup file (same as Next.js app)
- [ ] Step 9.2: Create `vitest.config.ts` with:
  - `environment: "jsdom"`
  - `coverage.provider: "v8"`
  - `coverage.reporter: ["lcov", "text"]`
  - `coverage.thresholds: { lines: 70 }` (matches Next.js threshold)
  - Path aliases for `@/`
- [ ] Step 9.3: Write unit tests using `@amiceli/vitest-cucumber` consuming
...
- [ ] Step 9.4: Run `npx vitest run --coverage` and verify LCOV output at
      `coverage/lcov.info`
- [ ] Step 9.5: Run `rhino-cli test-coverage validate coverage/lcov.info 70` and
      verify it passes

**Phase 9 Validation:**

```gherkin
Scenario: Phase 9 unit tests and coverage complete
...
- [ ] `nx test:quick demo-fe-ts-tanstack-start` passes (unit tests + coverage ≥70%)

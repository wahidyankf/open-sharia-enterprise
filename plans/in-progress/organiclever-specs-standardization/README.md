...
**Audience for `specs/apps/organiclever/`** — engineers AND product/project managers. Anything in `specs/` must be readable by a PM with no F#/Next.js/PGlite/XState/Effect TS background. Engineering jargon is allowed when necessary, but introduced with a one-line plain-language gloss the first time it appears in a file, and concepts always lead with intent ("what user problem this solves") before mechanism ("how the code is shaped"). The audience constraint shapes the structure of every new specs/ file in this plan.
...
7. `specs/apps/organiclever/README.md` includes a "For Product/Project Managers" reading-path section guiding non-engineering readers to the right files

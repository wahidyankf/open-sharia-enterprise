# Delivery Checklist: demo-be-csas
...
- [ ] Verify `rhino-cli test-coverage validate` supports LCOV (it does — already used by
      `organiclever-web`, `demo-be-exph`, `demo-be-pyfa`, `demo-be-rsax`, `demo-be-fsgi`)
...
- [ ] Confirm Coverlet XPlat Code Coverage with LCOV format works for C# tests (same mechanism
      as `demo-be-fsgi` which uses the identical collector)

---

## Phase 1: Project Scaffold

**Commit**: `feat(demo-be-csas): scaffold C#/ASP.NET Core project`

- [ ] Create `apps/demo-be-csas/` directory structure per tech-docs.md
...
**Commit**: `feat(demo-be-csas): add domain types and EF Core database layer`
...
**Commit**: `feat(demo-be-csas): add /health endpoint`
...
**Commit**: `feat(demo-be-csas): add register and login endpoints`
...
**Commit**: `feat(demo-be-csas): add token lifecycle and management endpoints`
...
**Commit**: `feat(demo-be-csas): add user account and security endpoints`
...
**Commit**: `feat(demo-be-csas): add admin endpoints`
...
**Commit**: `feat(demo-be-csas): add expense CRUD and currency handling`
...
**Commit**: `feat(demo-be-csas): add unit handling, reporting, and attachments`
...
**Commit**: `fix(demo-be-csas): achieve 90% coverage and pass quality gates`
...
- [ ] Validate: `rhino-cli test-coverage validate apps/demo-be-csas/coverage/**/coverage.info 90`
      passes
...
- [ ] `nx run demo-be-csas:test:quick` passes all gates
- [ ] Commit
...
**Commit**: `feat(infra): add demo-be-csas docker-compose dev environment`

- [ ] Create `infra/dev/demo-be-csas/Dockerfile.be.dev` (mcr.microsoft.com/dotnet/sdk:9.0-alpine + dotnet-ef tool) per tech-docs.md
- [ ] Create `infra/dev/demo-be-csas/docker-compose.yml` with PostgreSQL 17 + app per
      tech-docs.md
- [ ] Create `infra/dev/demo-be-csas/docker-compose.e2e.yml` (E2E overrides: detach mode,
      wait-for-healthy)
- [ ] Create `infra/dev/demo-be-csas/README.md` with startup instructions
...
**Commit**: `ci: add e2e-demo-be-csas GitHub Actions workflow`

- [ ] Create `.github/workflows/e2e-demo-be-csas.yml`:
...
**Commit**: `ci: add demo-be-csas coverage upload to main-ci`

- [ ] Verify existing `.NET SDK` `actions/setup-dotnet@v4` step in `main-ci.yml` covers .NET 9
      (it should — already added for `demo-be-fsgi`)
- [ ] Add coverage upload step for `apps/demo-be-csas/coverage/**/coverage.info`
      with flag `demo-be-csas`
...
**Commit**: `docs: add demo-be-csas to project documentation`

- [ ] Update `CLAUDE.md`:
  - Add `demo-be-csas` to Current Apps list with description
    (`demo-be-csas` — C#/ASP.NET Core REST API backend)
  - Add C# coverage info to coverage section
    (`demo-be-csas` enforces ≥90% via Coverlet LCOV + `rhino-cli test-coverage validate`)
  - Add `demo-be-csas` to `test:integration` caching note
    (Reqnroll + WebApplicationFactory + SQLite in-memory)
- [ ] Update `README.md`:
  - Add demo-be-csas badge in demo apps section
...
- [ ] Update `apps/demo-be-e2e/project.json`:
  - Add `demo-be-csas` to `implicitDependencies`

---

## Phase 15: Final Validation

- [ ] `nx run demo-be-csas:test:quick` passes (all integration scenarios, ≥90% coverage,
      zero format violations, zero analyzer warnings)
- [ ] `nx run demo-be-csas:test:unit` passes
- [ ] `nx run demo-be-csas:test:integration` passes — all 76 scenarios
- [ ] `nx run demo-be-csas:lint` passes
- [ ] `nx run demo-be-csas:typecheck` passes
- [ ] `nx run demo-be-csas:build` produces working artifact in `dist/`
- [ ] Docker Compose stack starts and health check passes (requires manual verification)
- [ ] `e2e-demo-be-csas.yml` workflow green (requires CI push)

...
matching `demo-be-jasb`'s MockMvc approach and `demo-be-fsgi`'s TickSpec approach). The
...
```json
{
  ...
  "sourceRoot": "apps/demo-be-csas/src",
    ...
    "test:quick": {
      ...
      "options": {
        "commands": [
          ...
          "(cd ../../ && apps/rhino-cli/rhino-cli test-coverage validate apps/demo-be-csas/coverage/**/coverage.info 90)",
...
> safety — consistent with the F# approach in `demo-be-fsgi`.
...
| demo-be-csas | 8201 (same port — mutually exclusive alternatives) |

### Docker Compose (`infra/dev/demo-be-csas/docker-compose.yml`)
...
services:
  demo-be-db:
    ...
    environment:
      POSTGRES_DB: demo_be_csas
      POSTGRES_USER: demo_be_csas
      POSTGRES_PASSWORD: demo_be_csas
    ports:
      - "5432:5432"
    volumes:
      - demo-be-csas-db-data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U demo_be_csas"]
      interval: 5s
      timeout: 3s
      retries: 5
    networks:
      - demo-be-csas-network

  demo-be-csas:
    build:
      context: .
      dockerfile: Dockerfile.be.dev
    container_name: demo-be-csas
    ports:
      - "8201:8201"
    environment:
      - ASPNETCORE_URLS=http://+:8201
      - DATABASE_URL=Host=demo-be-db;Database=demo_be_csas;Username=demo_be_csas;Password=demo_be_csas
      - APP_JWT_SECRET=dev-jwt-secret-at-least-32-chars-long
    volumes:
      - ../../../apps/demo-be-csas:/workspace:rw
...
### New Workflow: `e2e-demo-be-csas.yml`
...
The `.NET SDK` setup step already exists for `demo-be-fsgi`. Add only the coverage upload step:

```yaml
- name: Upload coverage — demo-be-csas
  uses: codecov/codecov-action@v5
  with:
    token: ${{ secrets.CODECOV_TOKEN }}
    files: apps/demo-be-csas/coverage/**/coverage.info
    flags: demo-be-csas

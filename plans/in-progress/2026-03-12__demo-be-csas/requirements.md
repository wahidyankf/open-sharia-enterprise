...
- **Integration tests** (`test:integration`): Reqnroll step definitions with
  ...
  monorepo convention established by `demo-be-jasb` (MockMvc + InMemoryDataStore),
  `demo-be-exph` (InMemoryStore contexts), `demo-be-fsgi` (TickSpec + WebApplicationFactory +
  SQLite in-memory), `demo-be-gogn` (Godog + httptest), `demo-be-pyfa` (pytest-bdd +
  TestClient), `demo-be-rsax` (Cucumber + Tower TestClient), and `demo-be-ktkt`

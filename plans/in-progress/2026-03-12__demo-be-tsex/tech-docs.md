...
determinism. This matches the pattern established by `demo-be-rsax` (cucumber + Tower
TestClient + in-memory stores) and `demo-be-ktkt` (Cucumber JVM + Ktor testApplication +
SQLite in-memory).
...
```json
{
  ...
  "sourceRoot": "apps/demo-be-tsex/src",
    ...
    "test:quick": {
      ...
      "options": {
        "commands": [
          ...
          "apps/rhino-cli/rhino-cli test-coverage validate apps/demo-be-tsex/coverage/lcov.info 90",
          "npx tsc --noEmit",
          "npx oxlint ."
        ],
        "parallel": false,
...
| demo-be-tsex | 8201 (same port — mutually exclusive alternatives) |

### Docker Compose (`infra/dev/demo-be-tsex/docker-compose.yml`)
...
services:
  demo-be-db:
    ...
    environment:
      POSTGRES_DB: demo_be_tsex
      POSTGRES_USER: demo_be_tsex
      POSTGRES_PASSWORD: demo_be_tsex
    ports:
      - "5432:5432"
    volumes:
      - demo-be-tsex-db-data:/var/lib/postgresql/data
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U demo_be_tsex"]
      interval: 5s
      timeout: 3s
      retries: 5
    networks:
      - demo-be-tsex-network

  demo-be-tsex:
    build:
      context: .
      dockerfile: Dockerfile.be.dev
    container_name: demo-be-tsex
    ports:
      - "8201:8201"
    environment:
      - DATABASE_URL=postgresql://demo_be_tsex:demo_be_tsex@demo-be-db:5432/demo_be_tsex
      - APP_JWT_SECRET=dev-jwt-secret-at-least-32-chars-long
    volumes:
      - ../../../apps/demo-be-tsex:/workspace:rw
    depends_on:
      demo-be-db:
...
### New Workflow: `e2e-demo-be-tsex.yml`

Mirrors `e2e-demo-be-pyfa.yml` with:
...
Add a coverage upload step for demo-be-tsex after the existing upload steps:

```yaml
- name: Upload coverage — demo-be-tsex
  uses: codecov/codecov-action@v5
  with:
    token: ${{ secrets.CODECOV_TOKEN }}
    files: apps/demo-be-tsex/coverage/lcov.info
    flags: demo-be-tsex

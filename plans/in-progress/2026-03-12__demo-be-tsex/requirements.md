...
- **Integration tests** (`test:integration`): Cucumber.js step definitions using the Effect TS
  ...
  convention established by `demo-be-jasb` (MockMvc + InMemoryDataStore), `demo-be-pyfa`
  (TestClient + SQLite in-memory), and `demo-be-rsax` (cucumber + Tower TestClient +
  in-memory stores).

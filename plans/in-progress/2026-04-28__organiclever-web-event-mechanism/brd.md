...
| Migration runner is idempotent | First run on empty DB applies v1; second run is a no-op; partial failure rolls back inside a transaction                                                                  |
...
| Migration applies twice / leaves DB half-migrated                                                                      | Low        | High   | `_migrations` tracking table; runner wraps each migration in `db.transaction(...)` so partial failure rolls back; idempotency tested in `test:integration`                                  |

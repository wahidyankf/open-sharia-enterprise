...
graph TB
    subgraph "Source (Tool-Agnostic)"
        AgentDocs["rules/agents/content/<br/>45 agent definitions<br/>(markdown + frontmatter)"]:::blue
        SkillDocs["rules/agents/skills/<br/>23 skill definitions<br/>(markdown + frontmatter)"]:::blue
    end
...
```
rules/agents/skills/{category}-{skill-name}/SKILL.md
```
...
def sync_all_agents():
    ...
    errors = validate_source_definitions('rules/agents/content/')
    if errors:
        print_errors(errors)
        sys.exit(1)
...
def validate_agent_definition(filepath: Path) -> List[str]:
    ...
    for skill in skills:
        skill_path = Path(f'rules/agents/skills/{skill}/SKILL.md')
...
**Purpose**: One-time migration from `.claude/agents/` to `rules/agents/content/`
...
def extract_agents():
    ...
    target_dir = Path('rules/agents/content')
...
**Purpose**: Copy `.claude/skills/` directory structure to `rules/agents/skills/`
...
def extract_skills():
    ...
    To:   rules/agents/skills/{skill-name}/SKILL.md
    """
    source_dir = Path('.claude/skills')
    target_dir = Path('rules/agents/skills')
    target_dir.mkdir(parents=True, exist_ok=True)
...
if [ -n "$MODIFIED_GENERATED" ]; then
  ...
  echo "  - Agents: rules/agents/content/"
  echo "  - Skills: rules/agents/skills/"
  echo ""
...
observer = Observer()
observer.schedule(AgentWatcher(), 'rules/agents/content/', recursive=False)
observer.start()
```
...
def test_full_sync_workflow(tmp_path):
    ...
    source = tmp_path / 'rules/agents/content/test.md'
    write_test_agent(source)

    # Execute: Run sync
    sync_all_agents(source_dir=tmp_path / 'rules/agents/content',
...
| `rules/agents/README.md`                             | New file: canonical agent catalog                            |
| `rules/agents/skills/README.md`                      | New file: canonical skills catalog                           |
| `rules/agents/meta/ex-ag-me__architecture.md`        | New file: agent architecture documentation                   |
...
**Source of truth**: `rules/agents/content/` (45 tool-agnostic definitions)
**Generated formats**:
...
See [Agent Architecture](./rules/agents/meta/ex-ag-me__architecture.md) for details.
```
...
**APPROVED**: `rules/agents/README.md` as canonical source

**Architecture**:

- **Source**: `rules/agents/README.md` (canonical, manually edited)
...
| ------------------- | ----------------------------------------------- | ---------------------------------------- |
| `agent-maker`       | Creates in `.claude/agents/`                    | Create in `rules/agents/content/`        |
| `wow-rules-checker` | Validates `.claude/agents/`, `.opencode/agent/` | Validate `rules/agents/`, skip generated |
| `wow-rules-fixer`   | Fixes in `.claude/agents/` (uses Bash tools)    | Fix in `rules/agents/`, skip generated   |
...
**New behavior**: Creates agents in `rules/agents/content/` using tool-agnostic format
...
1. **Create source definition** in `rules/agents/content/{agent-name}.md`
...
**New behavior**: Validates `rules/agents/content/` (source), detects edits to generated directories as errors

**Implementation changes**:

1. **Validation targets**:
   - ✅ Validate: `rules/agents/content/*.md`
   - ✅ Validate: `rules/agents/skills/*.md`
...
**New behavior**: Modifies `rules/agents/content/` (source), skips findings referencing generated files
...
1. **Fix source, not generated**:
   ...
   1. **Check if source or generated**:
      ...
      - If path starts with `rules/agents/` → FIX (source)
      - If path starts with `rules/agents/skills/` → FIX (source)

   2. **Apply fix to source**:
      - Use Edit tool (not Bash, since docs/ can use Edit)
      - Modify `rules/agents/content/{agent-name}.md`

   3. **Instruct sync**:
   ```

   ✅ Fixed: rules/agents/content/docs-maker.md
...
2. **Skip generated files with warning**:
   ...
   if should_skip_finding(finding.filepath):
       ...
       log_info(f"Edit source instead: rules/agents/content/...")
       continue
   ```
...
**New behavior**: Reference `rules/agents/content/` as source location
...
**New behavior**: Explicitly validates source (`rules/`) only, skips generated directories
...
| ---------------------- | ------------------------------------------------ | --------------------------------------------- |
| **Source location**    | `rules/agents/content/`                          | Documentation co-location, tool-agnostic      |

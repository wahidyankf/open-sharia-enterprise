...
graph TB
    subgraph "Source (Tool-Agnostic)"
        AgentDocs["governance/agents/content/<br/>45 agent definitions<br/>(markdown + frontmatter)"]:::blue
        SkillDocs["governance/agents/skills/<br/>23 skill definitions<br/>(markdown + frontmatter)"]:::blue
    end
...
└── agents/                                # Layer 4: AI Agents
    ...
    │   ├── docs-applying-content-quality/ # 23 skill definitions (folder/SKILL.md)
    │   │   └── SKILL.md
...
```
governance/agents/skills/{category}-{skill-name}/SKILL.md
```
...
def sync_all_agents():
    ...
    errors = validate_source_definitions('governance/agents/content/')
    if errors:
        print_errors(errors)
        sys.exit(1)
...
def validate_agent_definition(filepath: Path) -> List[str]:
    ...
    for skill in skills:
        skill_path = Path(f'governance/agents/skills/{skill}/SKILL.md')
...
**Purpose**: One-time migration from `.claude/agents/` to `governance/agents/content/`
...
def extract_agents():
    ...
    target_dir = Path('governance/agents/content')
...
**Purpose**: Copy `.claude/skills/` directory structure to `governance/agents/skills/`
...
def extract_skills():
    ...
    To:   governance/agents/skills/{skill-name}/SKILL.md
    """
    source_dir = Path('.claude/skills')
    target_dir = Path('governance/agents/skills')
    target_dir.mkdir(parents=True, exist_ok=True)
...
if [ -n "$MODIFIED_GENERATED" ]; then
  ...
  echo "  - Agents: governance/agents/content/"
  echo "  - Skills: governance/agents/skills/"
  echo ""
...
**Current**: Manual test needed (no validate-agents.py yet - using ./repo-cli agents validate)
...
observer = Observer()
observer.schedule(AgentWatcher(), 'governance/agents/content/', recursive=False)
observer.start()
```
...
def test_full_sync_workflow(tmp_path):
    ...
    source = tmp_path / 'governance/agents/content/test.md'
    write_test_agent(source)

    # Execute: Run sync
    sync_all_agents(source_dir=tmp_path / 'governance/agents/content',
...
| `governance/development/agents/ai-agents.md`              | Add "Source of Truth" section, document tool-agnostic format |
| `governance/ex-ru__repository-governance-architecture.md` | Update Layer 4 to reference docs source                      |
| `.claude/agents/README.md`                                | Add generated banner, link to source                         |
| `.opencode/agent/README.md`                               | Add generated banner, link to source                         |
| `governance/agents/README.md`                             | New file: canonical agent catalog                            |
| `governance/agents/skills/README.md`                      | New file: canonical skills catalog                           |
| `governance/agents/meta/ex-ag-me__architecture.md`        | New file: agent architecture documentation                   |
...
**Source of truth**: `governance/agents/content/` (45 tool-agnostic definitions)
**Generated formats**:
...
See [Agent Architecture](./governance/agents/meta/ex-ag-me__architecture.md) for details.
```
...
**APPROVED**: `governance/agents/README.md` as canonical source

**Architecture**:

- **Source**: `governance/agents/README.md` (canonical, manually edited)
...
**New behavior**: Creates agents in `governance/agents/content/` using tool-agnostic format
...
1. **Create source definition** in `governance/agents/content/{agent-name}.md`
...
**New behavior**: Validates `governance/agents/content/` (source), detects edits to generated directories as errors

**Implementation changes**:

1. **Validation targets**:
   - ✅ Validate: `governance/agents/content/*.md`
   - ✅ Validate: `governance/agents/skills/*.md`
...
**New behavior**: Modifies `governance/agents/content/` (source), skips findings referencing generated files
...
1. **Fix source, not generated**:
   ...
   1. **Check if source or generated**:
      ...
      - If path starts with `governance/agents/` → FIX (source)
      - If path starts with `governance/agents/skills/` → FIX (source)

   2. **Apply fix to source**:
      - Use Edit tool (not Bash, since docs/ can use Edit)
      - Modify `governance/agents/content/{agent-name}.md`

   3. **Instruct sync**:
   ```

   ✅ Fixed: governance/agents/content/docs-maker.md
...
2. **Skip generated files with warning**:
   ...
   if should_skip_finding(finding.filepath):
       ...
       log_info(f"Edit source instead: governance/agents/content/...")
       continue
   ```

### Wow-Rules-Maker Updates

**Current behavior**: May reference `.claude/agents/` in created governance/conventions

**New behavior**: Reference `governance/agents/content/` as source location
...
**New behavior**: Explicitly validates source (`governance/`) only, skips generated directories
...
| ---------------------- | ------------------------------------------------ | --------------------------------------------- |
| **Source location**    | `governance/agents/content/`                     | Documentation co-location, tool-agnostic      |

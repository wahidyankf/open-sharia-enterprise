...
A governance documentation update and targeted agent tier correction. Phases 1-3 (done):
document budget-adaptive inheritance, extend model-selection.md to dual-platform coverage,
fix CLAUDE.md plan format. Phases 4-9 (pending): create a cited benchmark reference
document, correct 8 agent tier assignments (7 OMIT→SONNET, 1 SONNET→HAIKU), add benchmark
citations to policy docs, validate, sync, archive.
...
Feature: Validations pass after changes
  ...
  Scenario: Claude agent validation passes
    Given all .claude/agents/ files including the 8 updated by Phase 5
...
- Update `repo-governance/development/agents/model-selection.md` _(done)_:

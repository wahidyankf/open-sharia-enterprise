...
| `.eslintrc.json`                          | _(removed)_                                    | Replaced by `oxlint.json` identical to `organiclever-web`'s                                                                                                                                                                                                                                                                                                                                          |
| `.prettierrc`                             | _(removed)_                                    | Use repo-root Prettier config                                                                                                                                                                                                                                                                                                                                                                        |
| `README.md`                               | `README.md`                                    | Rewrite to match `apps/organiclever-web/README.md` structure                                                                                                                                                                                                                                                                                                                                         |
...
| `.dockerignore`                              | Match `organiclever-web/.dockerignore`                                          |
| `Dockerfile`                                 | Multi-stage Next.js standalone build (copy from `organiclever-web/Dockerfile`)  |
...
| `apps/wahidyankf-web-e2e/` (entire project)  | Playwright-BDD E2E runner, modelled on `organiclever-web-e2e/`                  |
...
`organiclever-web` uses a different pin (e.g., `jsdom ^28.1.0`,
...
| -------------------------- | ---------- | ----------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `next`                     | `14.2.13`  | `16.1.6`    | `ayokoding-web`, `oseplatform-web`, `organiclever-web` (all `16.1.6`) | Exact pin across all three Next.js apps.                                                                                                                                              |
| `react`                    | `^18`      | `^19.0.0`   | `ayokoding-web`, `oseplatform-web` (`^19.0.0`)                        | `organiclever-web` uses `^19.1.0`; the `^19.0.0` caret resolves to the same installed version, so adopting `^19.0.0` matches both content platforms exactly without functional drift. |
...
| `jsdom`                         | `^22.0.0`            | `^26.0.0`                                                        | `ayokoding-web`, `oseplatform-web` (`^26.0.0`)                                                                             | Exact match; Vitest-4-compatible. (organiclever-web uses `^28.1.0` — we take the content-platform value per the stack-parity rule.)                                                                                                   |
...
| `vite-tsconfig-paths`           | _(new)_              | `^5.0.0`                                                         | `ayokoding-web`, `oseplatform-web` (`^5.0.0`)                                                                              | Exact match. (organiclever-web uses `^6.1.1` — we take the content-platform value.)                                                                                                                                                   |
| `vitest`                        | `^0.31.0`            | `^4.0.0`                                                         | `ayokoding-web`, `oseplatform-web` (`^4.0.0`). (`organiclever-web` is at `^4.1.0` — we align to content-platform siblings) | vitest: two content-platform siblings (ayokoding-web, oseplatform-web) pin `^4.0.0`; `organiclever-web` carries `^4.1.0`. Target `^4.0.0` aligns to the content-platform pattern (wahidyankf-web is structurally a content platform). |

### E2E sibling app (`apps/wahidyankf-web-e2e`) dependencies

Model after `apps/organiclever-web-e2e/package.json` — the three content
...
so a single `-e2e` project suffices, mirroring `organiclever-web-e2e`).

| Package                | Target    | Matches sibling                    | Rationale                                      |
| ---------------------- | --------- | ---------------------------------- | ---------------------------------------------- |
| `@playwright/test`     | `^1.52.0` | `organiclever-web-e2e` (`^1.52.0`) | Matches the E2E model sibling directly.        |
| `playwright-bdd`       | `^8.4.2`  | `organiclever-web-e2e` (`^8.4.2`)  | BDD glue layer used repo-wide.                 |
| `@axe-core/playwright` | `^4.10.1` | `organiclever-web-e2e` (`^4.10.1`) | Accessibility scans for the a11y feature file. |
...
Targets exposed, modelled on `apps/organiclever-web/project.json` minus
`codegen` (no OpenAPI contract):

- `dev` — `next dev --port 3201` (port rationale: `3100` oseplatform-web,
  `3101` ayokoding-web, `3200` organiclever-web; `3201` first free above
  organiclever-web; recorded in top-level `CLAUDE.md` under the app's section).
...
Model on `apps/organiclever-web-e2e/project.json`. Targets: `install`,
...
`repo-governance/development/infra/nx-targets.md` currently enumerates allowed
...
- `repo-governance/development/infra/nx-targets.md` → append the app and
...
the same feature files — this matches the `organiclever-web` pattern.
...
- **Coverage thresholds (in `vitest.config.ts`)** — lines 80, functions
  ...
  floor in `test:quick`. The 70% threshold carried by `organiclever-web`

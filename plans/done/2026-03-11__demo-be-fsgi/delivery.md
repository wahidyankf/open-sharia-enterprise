...
- [x] Create `apps/demo-be-fsgi/` directory structure per tech-docs.md
...
- [x] Create `infra/dev/demo-be-fsgi/Dockerfile.be.dev` (.NET 10 SDK Alpine)
- [x] Create `infra/dev/demo-be-fsgi/docker-compose.yml` with PostgreSQL + app
- [x] Create `infra/dev/demo-be-fsgi/docker-compose.e2e.yml` (E2E overrides)
- [x] Create `infra/dev/demo-be-fsgi/README.md` with startup instructions
...
- [x] Create `.github/workflows/e2e-demo-be-fsgi.yml`:
...
- [x] Add coverage upload step for `apps/demo-be-fsgi/coverage/altcov.info`
...
- [x] `nx run demo-be-fsgi:test:quick` passes (157 integration + 57 unit, 95.46% coverage, lint clean)
- [x] `nx run demo-be-fsgi:test:unit` passes
- [x] `nx run demo-be-fsgi:test:integration` passes — all 76+ scenarios (157 test cases with outlines)
- [x] `nx run demo-be-fsgi:lint` passes
- [x] `nx run demo-be-fsgi:typecheck` passes
- [x] `nx run demo-be-fsgi:build` produces working artifact
- [ ] Docker Compose stack starts and health check passes (requires manual verification)
- [ ] `e2e-demo-be-fsgi.yml` workflow green (requires CI push)

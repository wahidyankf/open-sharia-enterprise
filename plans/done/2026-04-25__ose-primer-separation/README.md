...
- **Parent-level worktree rule**: Per the [Subrepo Worktree Workflow Convention](https://github.com/wahidyankf/ose-projects/blob/main/governance/conventions/structure/subrepo-worktrees.md) (parent-level), worktrees are mandatory when a plan's Scope names `ose-public` **from a parent-rooted session**. This plan lives inside `ose-public/plans/` and is executed from inside `ose-public` directly, so the parent-level worktree rule does not apply here.
...
- [Subrepo Worktree Workflow Convention](https://github.com/wahidyankf/ose-projects/blob/main/governance/conventions/structure/subrepo-worktrees.md) — parent-level rule (not applicable here; explained in Scope).

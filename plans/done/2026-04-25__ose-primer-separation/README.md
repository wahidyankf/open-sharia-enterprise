...
**Status**: In Progress
**Created**: 2026-04-18
**Structure**: Multi-file (five documents — see [Plans Organization Convention](../../../repo-governance/conventions/structure/plans.md))
...
**The separation step**: The `a-demo-*` polyglot showcase (11 backend demos spanning Go/Java/Python/Rust/Elixir/F#/Kotlin/C#/Clojure/TypeScript/Java-Vertx, 3 frontend variants, a fullstack example, the shared OpenAPI contracts spec, the demo E2E suites, and all associated GitHub Actions workflows) is the canonical reason `ose-primer` exists as a template. Keeping two copies — one in `ose-public`, one in `ose-primer` — doubles maintenance cost, doubles CI burden, and creates silent drift between the template and the source. The primer already contains the demos (it was derived from `ose-public`), so this plan **extracts the demos from `ose-public`** as a one-time event. After extraction, `ose-primer` is the authoritative home of the polyglot showcase; `ose-public` is the authoritative home of product apps (OrganicLever, AyoKoding, OSE Platform) and the repo-governance/skills/agents layer that both repos share. The bidirectional sync then keeps the shared layer aligned; the demo layer lives in one place.
...
1. **Awareness layer** — Cross-references so `ose-public` contributors and AI agents know `ose-primer` exists: edits to `README.md`, `CLAUDE.md`, `AGENTS.md`; new `docs/reference/related-repositories.md`; new `repo-governance/conventions/structure/ose-primer-sync.md` policy doc.
...
4. **Workflow orchestrations** — Two new workflow documents under `repo-governance/workflows/repo/`:
   ...
   - `repo-ose-primer-extraction-execution.md` — the one-time extraction orchestrator: parity-check gate → catch-up propagation loop on failure → Commit A–J sequence → post-extraction verification. Pattern-reusable if a future plan extracts another app family (e.g., extract product-specific content to a separate repo), though the current extraction scope is hard-coded to `a-demo-*` and `specs/apps/a-demo/`.
     Both workflows conform to the [Workflow Naming Convention](../../../repo-governance/conventions/structure/workflow-naming.md) — scope=`repo`, qualifier=`ose-primer-sync` / `ose-primer-extraction`, type=`execution` (forward procedure per invocation, not iterative zero-finding termination). Workflow bodies follow [repo-defining-workflows](../../../.claude/skills/repo-defining-workflows/SKILL.md) patterns: YAML frontmatter (name, goal, termination, inputs, outputs), phase breakdown, agent coordination, Gherkin success criteria.
...
10. **Post-extraction cleanup** — Re-verify that product apps (OrganicLever, AyoKoding, OSE Platform), `rhino-cli` (trimmed), `libs/golang-commons`, and all generic repo-governance/skills/agents remain green after the removal: `npm install`, `npm run doctor`, `nx affected -t typecheck lint test:quick spec-coverage`, product E2E, markdown lint, `nx run rhino-cli:test:quick`. Any lingering demo-reference gets caught here. Part of the extraction workflow's terminal phase.
11. **Library cleanup (Commit I of the extraction sequence)** — Delete four `libs/` entries that become orphans after demo removal: `libs/clojure-openapi-codegen/` (only consumer was the extracted Clojure backend), `libs/elixir-cabbage/`, `libs/elixir-gherkin/`, `libs/elixir-openapi-codegen/` (all three used only by the extracted Elixir backend). Update `libs/README.md`. Explicitly kept: `libs/golang-commons/` (consumed by all Go CLIs), `libs/hugo-commons/` (still imported by `ayokoding-cli` + `oseplatform-cli` despite Hugo sites being migrated — flagged for a future cleanup plan, not this one), `libs/ts-ui/` + `libs/ts-ui-tokens/` (consumed by `organiclever-web`).
...
- **New role token in the [Agent Naming Convention](../../../repo-governance/conventions/structure/agent-naming.md)** — Analysed in `tech-docs.md`; conclusion is that `maker` suffices because both agents produce proposal artifacts. No convention amendment required.
...
- **Parent-level worktree rule**: Per the [Subrepo Worktree Workflow Convention](https://github.com/wahidyankf/ose-projects/blob/main/repo-governance/conventions/structure/subrepo-worktrees.md) (parent-level), worktrees are mandatory when a plan's Scope names `ose-public` **from a parent-rooted session**. This plan lives inside `ose-public/plans/` and is executed from inside `ose-public` directly, so the parent-level worktree rule does not apply here.
...
- **After extraction, `apps/a-demo-*` and `specs/apps/a-demo/` cease to exist in `ose-public`.** The classifier tags these paths `neither (extracted 2026-04-18; ose-primer is authoritative)` so future runs of either agent skip them.
- **`ose-public` commits land directly on `main`** (Trunk Based Development). Every Phase 1–12 commit — including the ten Phase 8 extraction commits (A → J) — is made on `main` and pushed to `origin/main` as it lands. No feature branch is created for any plan phase. No PR is opened against `ose-public`. Husky pre-commit + pre-push hooks are the quality gate. Environment branches (`prod-ayokoding-web`, `prod-oseplatform-web`, `prod-organiclever-web`) remain Vercel-deployment-only; this plan never touches them.
- **`ose-primer` commits ALWAYS go through a PR.** Every mutation of `ose-primer` — propagation runs, catch-up runs during the Phase 7 parity loop, any future adoption-apply-back, any future classifier-scoped primer update — uses the propagation-maker's apply mode: git worktree inside the primer clone → new branch → commit → push → `gh pr create --draft` → human review and merge. No agent and no workflow may commit directly to `wahidyankf/ose-primer:main`. This asymmetry is absolute and has no escape hatch; violations are governance issues regardless of justification. See `tech-docs.md` §Safety Rules Rules 9–10 for the full statement.
...
| Dangling demo references in repo-governance/docs/config survive extraction            | High         | Post-extraction `grep -r 'a-demo' --include='*.md' --include='*.yml' --include='*.json' ose-public/` MUST return zero matches outside archived plans and the sync convention's classifier table.                                                                                                                                                               |
...
| Go `go.work` and .NET `.sln` files drift from actual repo contents                    | Medium       | Extraction removes demo module references from both files in a dedicated Commit E ("prune root configs"), three commits after Commit B (app directory deletion). `go.work` and `.sln` reference deleted paths between Commits B and D — do not run `go work sync` or `dotnet sln list` until after Commit E lands.                                             |
...
- **Post-extraction `grep` sweep**: `grep -rnI 'a-demo' ose-public/ --include='*.md' --include='*.yml' --include='*.yaml' --include='*.json' --include='*.toml' --include='Brewfile' --include='*.sln' --include='go.work'` returns matches ONLY from `plans/done/` (archived), `plans/in-progress/2026-04-18__ose-primer-separation/` (this plan), and the `apps/a-demo-*` row of the classifier table inside `repo-governance/conventions/structure/ose-primer-sync.md`. Every other match is a dangling reference and MUST be resolved before Phase 8 is considered complete.
- **Post-extraction `nx graph`**: contains no project whose name starts with `a-demo-`; the graph renders without orphan edges.
- **Post-extraction product-app health**: `nx run organiclever-web:test:quick`, `nx run organiclever-be:test:quick`, `nx run ayokoding-web:test:quick`, `nx run oseplatform-web:test:quick`, `nx run rhino-cli:test:quick` all pass.
...
- [Plans Organization Convention](../../../repo-governance/conventions/structure/plans.md) — five-document multi-file structure rule.
- [Agent Naming Convention](../../../repo-governance/conventions/structure/agent-naming.md) — scope/qualifier/role filename rule.
- [Maker-Checker-Fixer Pattern](../../../repo-governance/development/pattern/maker-checker-fixer.md) — three-stage quality workflow.
- [Licensing Convention](../../../repo-governance/conventions/structure/licensing.md) — FSL vs MIT split.
- [Subrepo Worktree Workflow Convention](https://github.com/wahidyankf/ose-projects/blob/main/repo-governance/conventions/structure/subrepo-worktrees.md) — parent-level rule (not applicable here; explained in Scope).

...
graph TD
    SPECS["specs/apps/ose-app/<br/>OpenAPI + Gherkin + DDD"]:::teal

    WEB["apps/ose-app-web<br/>Next.js 16 + tRPC<br/>port 3300"]:::blue
    BE["apps/ose-app-be<br/>F#/Giraffe on .NET 10<br/>EF Core + DbUp<br/>port 8302"]:::blue

    WEBE2E["apps/ose-app-web-e2e<br/>Playwright-BDD"]:::orange
    BEE2E["apps/ose-app-be-e2e<br/>Playwright-BDD"]:::orange

    PG["PostgreSQL 17<br/>infra/dev/ose-app/<br/>docker-compose"]:::purple
...
**Divergence from prior organiclever-be F#**: the prior `organiclever-be` F# shim (pre-Java migration) had health-only with no DB. ose-app-be wires DB and DbUp migrations from day 0 because GRC documents are persistent server-side state. Package set follows `ose-primer/apps/crud-be-fsharp-giraffe`.
...
**Choice**: Mirror `apps/organiclever-web/` Next.js 16 + App Router + TypeScript + tRPC + Tailwind v4 + Vitest + Storybook. **Remove** PGlite — ose-app-web is a server-first client. **Reuse** the shared component library `libs/web-ui` (workspace name `@open-sharia-enterprise/web-ui`) and design tokens at `libs/web-ui-token` (workspace name `@open-sharia-enterprise/web-ui-token`) — the same libraries `organiclever-web`, `oseplatform-web`, and `ayokoding-web` consume.
...
- `apps/ose-app-web/package.json` declares `"@open-sharia-enterprise/web-ui": "*"` and `"@open-sharia-enterprise/web-ui-token": "*"` (npm workspace symlinks resolve them).
- `apps/ose-app-web/next.config.ts` includes both packages in `transpilePackages: ["@open-sharia-enterprise/web-ui", "@open-sharia-enterprise/web-ui-token"]`.
- `apps/ose-app-web/src/app/globals.css` imports the **base** tokens via `@import "@open-sharia-enterprise/web-ui-token/src/tokens.css";`. The OrganicLever-specific warm OKLCH palette (`organiclever.css`) is **not** imported. A future plan introduces `ose-app.css` for GRC-specific theming if needed.
- `apps/ose-app-web/project.json` lists `"web-ui"` in `implicitDependencies` so `nx affected` re-runs the FE when shared components change.
- Smoke `page.tsx` imports at least one `web-ui` primitive (e.g., `<Button>`) so the wiring is verified end-to-end at bootstrap.

**Component additions to `libs/web-ui`**: none at bootstrap. If GRC needs new shared components later, they go through `swe-ui-maker` and land in `libs/web-ui/src/`, not in `apps/ose-app-web/`.

**Verification**:

- `grep -r '@electric-sql/pglite' apps/ose-app-web` returns zero matches (no PGlite).
- `grep -F '@open-sharia-enterprise/web-ui' apps/ose-app-web/package.json apps/ose-app-web/next.config.ts` returns at least one hit per file.
- `npx nx graph` shows an edge `ose-app-web → web-ui` and `ose-app-web → web-ui-token`.
- `apps/ose-app-web/src/app/page.tsx` contains an `import { ... } from "@open-sharia-enterprise/web-ui"` statement.

### DD-4: OpenAPI 3.1 as single source of truth for the FE↔BE contract

**Choice**: `specs/apps/ose-app/containers/contracts/openapi.yaml` is the single contract source. `ose-app-contracts` Nx project lints + bundles it via `@redocly/cli` and `@stoplight/spectral-cli` (same toolchain as `organiclever-contracts`). Downstream codegen:

| Consumer      | Tool                        | Generator flag             | Output path                                                           |
| ------------- | --------------------------- | -------------------------- | --------------------------------------------------------------------- |
| `ose-app-be`  | `npx openapi-generator-cli` | `-g fsharp-giraffe-server` | `apps/ose-app-be/generated-contracts/` (.fs files compiled in fsproj) |
| `ose-app-web` | `npx @hey-api/openapi-ts`   | n/a (TS-native)            | `apps/ose-app-web/src/generated-contracts/`                           |
...
**Choice**: Declare four contexts in `specs/apps/ose-app/ddd/bounded-contexts.yaml` at bootstrap. Each gets a `specs/apps/ose-app/ddd/ubiquitous-language/<context>.md` stub.
...
**Bootstrap scope**: YAML entries + `ubiquitous-language/<context>.md` stubs of ~10 lines each, listing canonical terms placeholders. F# folders `src/OseAppBe/Domain/<Context>.fs` are stubbed empty.
...
- `specs/apps/ose-app/behavior/be/gherkin/health.feature` — Scenario: BE health check returns 200.
- `specs/apps/ose-app/behavior/web/gherkin/smoke.feature` — Scenario: FE home page loads.

**BE consumption**: `ose-app-be:test:unit` runs xUnit + TickSpec; step files live at `apps/ose-app-be/tests/OseAppBe.Tests/Unit/HealthSteps.fs`.

**FE consumption**: `ose-app-web-e2e:test:e2e` runs Playwright-BDD; step files live at `apps/ose-app-web-e2e/steps/smoke.steps.ts`.

**spec-coverage validators** (`rhino-cli spec-coverage validate`) must report 100% step-coverage for both `.feature` files.

### DD-7: PR quality gate routing via `lang:fsharp` and `domain:ose-app` tags

**Choice**: Re-use existing `.github/workflows/pr-quality-gate.yml` detector. Tag each project:

| Project             | Tags                                                            |
| ------------------- | --------------------------------------------------------------- |
| `ose-app-web`       | `type:app`, `platform:nextjs`, `lang:ts`, `domain:ose-app`      |
| `ose-app-be`        | `type:app`, `platform:giraffe`, `lang:fsharp`, `domain:ose-app` |
| `ose-app-web-e2e`   | `type:e2e`, `platform:playwright`, `lang:ts`, `domain:ose-app`  |
| `ose-app-be-e2e`    | `type:e2e`, `platform:playwright`, `lang:ts`, `domain:ose-app`  |
| `ose-app-contracts` | `type:lib`, `platform:openapi`, `domain:ose-app`                |
...
**Choice**: Create `.github/workflows/test-and-deploy-ose-app-web-development.yml` mirroring `test-and-deploy-organiclever-web-development.yml`. Stub the deploy step to a no-op until Vercel project is linked in a later plan.

**Staging + prod workflows** (`test-ose-app-web-staging.yml`, `deploy-ose-app-web-to-production.yml`) are scaffolded as stubs (workflow_dispatch only, no scheduled trigger) so the naming convention is established; activation deferred.

### DD-9: doctor wires .NET 10 + local tools

**Choice**: Add `apps/ose-app-be/global.json` pinning SDK 10.x (mirror `ose-primer/apps/crud-be-fsharp-giraffe/global.json`). Add `apps/ose-app-be/dotnet-tools.json` declaring `altcover.global` and `fsharp-analyzers` (matching the verified contents of `ose-primer/apps/crud-be-fsharp-giraffe/dotnet-tools.json` — `[Repo-grounded]`, confirmed via `cat ose-primer/apps/crud-be-fsharp-giraffe/dotnet-tools.json`).
...
- `apps/ose-app-be/.env.example` — keys `OPENROUTER_API_KEY=`, `OPENROUTER_MODEL=openrouter/auto`, `OPENROUTER_BASE_URL=https://openrouter.ai/api/v1`.
- F# settings record `OseAppBe.AiOrchestration.OpenRouterSettings` in `src/OseAppBe/Domain/AiOrchestration.fs` with three fields (`apiKey`, `model`, `baseUrl`).
...
```
apps/
├── ose-app-web/                    # Full Next.js 16 scaffold mirroring organiclever-web minus PGlite
├── ose-app-be/                     # Full F#/Giraffe scaffold mirroring crud-be-fsharp-giraffe (DB-enabled)
├── ose-app-web-e2e/                # Mirror organiclever-web-e2e
└── ose-app-be-e2e/                 # Mirror organiclever-be-e2e
...
│       ├── .spectral.yaml
│       ├── openapi.yaml
│       ├── project.json             # ose-app-contracts Nx project
│       ├── paths/health.yaml
│       ├── schemas/error.yaml
│       └── schemas/health.yaml
├── ddd/
...
├── .env.example
├── .gitignore
├── docker-compose.yml               # postgres + ose-app-be + ose-app-web
├── docker-compose.ci.yml            # CI overrides
├── Dockerfile.be.dev
└── Dockerfile.fe.dev

.github/workflows/
├── test-and-deploy-ose-app-web-development.yml
├── test-ose-app-web-staging.yml      # Stub (workflow_dispatch only)
└── deploy-ose-app-web-to-production.yml # Stub (workflow_dispatch only)
```

### `apps/ose-app-be/` structure (mirrors `ose-primer/apps/crud-be-fsharp-giraffe/`)
...
│   └── OseAppBe.fsproj                     # Microsoft.NET.Sdk.Web, net10.0, TreatWarningsAsErrors=true
├── tests/OseAppBe.Tests/
...
│   └── OseAppBe.Tests.fsproj                # xunit + TickSpec + AltCover packages
...
### `apps/ose-app-web/` structure (mirrors `apps/organiclever-web/` minus PGlite)
...
│   │   ├── layout.tsx
│   │   ├── page.tsx                        # Smoke screen with "ose-app-web alive" heading
...
### `apps/ose-app-web-e2e/` and `apps/ose-app-be-e2e/` structure

Mirror `organiclever-web-e2e/` and `organiclever-be-e2e/` exactly. Swap project name and spec path:

- `apps/ose-app-web-e2e/playwright.config.ts` — `WEB_BASE_URL=http://localhost:3300`
- `apps/ose-app-be-e2e/playwright.config.ts` — `BASE_URL=http://localhost:8302`
- Step files: `apps/ose-app-*-e2e/steps/smoke.steps.ts` and `apps/ose-app-*-e2e/steps/health.steps.ts`
...
- **Nx implicit deps**: `ose-app-web` → `ose-app-contracts`, `rhino-cli`, `web-ui`, `web-ui-token`; `ose-app-be` → `ose-app-contracts`, `rhino-cli`; `ose-app-web-e2e` → `ose-app-web`, `ose-app-be`; `ose-app-be-e2e` → `ose-app-be`
...
- `infra/dev/ose-app/` — additive
- `specs/apps/ose-app/` — additive

No edits to `apps/organiclever-*`, `apps/ayokoding-*`, `apps/oseplatform-*`, `apps/wahidyankf-*`, `apps/rhino-cli/`, `libs/`. Rollback is mechanical.

## Open Questions

1. **Vercel project linkage** — When does ose-app-web get a Vercel project? Deferred; not blocked by this plan.

...
**For Agents**: Reference this documentation when writing TypeScript code. The `programming-typescript` skill provides quick access to these standards.
...
**See Also**: [Functional Programming](./ex-so-stla-ts__functional-programming.md) for pure functions patterns, [Best Practices](./ex-so-stla-ts__best-practices.md) for explicit coding standards, [Type Safety](./ex-so-stla-ts__type-safety.md) for immutable type patterns.

...
- **[Reproducibility First](../principles/software-engineering/ex-pr-se__reproducibility.md)**: All environment configuration is explicit, version-controlled, and reproducible. Eliminates "works on my machine" problems through deterministic setup.

- **[Automation Over Manual](../principles/software-engineering/ex-pr-se__automation-over-manual.md)**: Environment setup automated through version managers (Volta), lockfiles, scripts, and containers. Manual setup steps eliminated or documented.

- **[Explicit Over Implicit](../principles/software-engineering/ex-pr-se__explicit-over-implicit.md)**: Runtime versions pinned explicitly (package.json volta field). Dependencies locked with exact versions (package-lock.json). No implicit system dependencies.

- **[Simplicity Over Complexity](../principles/general/ex-pr-ge__simplicity-over-complexity.md)**: Use simple, proven tools (Volta, npm lockfiles, Docker) instead of complex custom solutions. Minimum configuration for maximum reproducibility.

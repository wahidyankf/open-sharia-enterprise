# OSE GRC API Contract

OpenAPI 3.1 specification for the OSE GRC (Governance, Risk, and Compliance) platform REST API.

## Purpose

This contract defines the exact shape of every request and response for the OSE GRC backend
(`ose-grc-be`) and frontend (`ose-grc-web`). It is the **single source of truth** for API
...
nx run ose-grc-contracts:lint

# Bundle into single resolved YAML + JSON
nx run ose-grc-contracts:bundle

# Generate browsable API documentation
nx run ose-grc-contracts:docs
# Open specs/apps/ose-grc/containers/contracts/generated/docs/index.html
```
...
2. Run `nx run ose-grc-contracts:lint` to validate
3. Run `nx run ose-grc-contracts:bundle` to regenerate the bundled spec
...
`ose-grc-be` and `ose-grc-web`. This ensures that re-running codegen (which changes the

# OSE GRC Application Specs

Platform-agnostic specifications for the OSE GRC (Governance, Risk, and Compliance) fullstack
...
| --------- | --------------------------------------- | -------------------------- | --------------------------------------------------------- | ------- | ---------------------------------------- |
| `be`      | HTTP-semantic (GET, POST, status codes) | `Given the API is running` | [behavior/be/gherkin/](./behavior/be/gherkin/README.md)   | health  | `apps/ose-grc-be` (F#/Giraffe, TickSpec) |
| `web`     | UI-semantic (clicks, types, sees)       | `Given the app is running` | [behavior/web/gherkin/](./behavior/web/gherkin/README.md) | smoke   | `apps/ose-grc-web` (Next.js 16)          |

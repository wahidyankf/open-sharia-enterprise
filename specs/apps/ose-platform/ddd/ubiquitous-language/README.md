...
Per-bounded-context glossaries for `ose-web`. Every term used in a Gherkin feature
...
`rhino-cli ddd ul ose-platform` checks each glossary:
...
5. Run `rhino-cli ddd ul ose-platform` to confirm validity before committing.

Feature: User Registration
  ...
  Scenario: Reject registration when email already exists
    Given a user "alice" is registered with email "alice@example.com" and password "Str0ng#Pass1"
    When the client sends POST /api/v1/auth/register with body { "username": "newuser", "email": "alice@example.com", "password": "Str0ng#Pass1" }
    Then the response status code should be 409
  ...
  Scenario: Reject registration with empty username
    When the client sends POST /api/v1/auth/register with body { "username": "", "email": "alice@example.com", "password": "Str0ng#Pass1" }
  ...
  Scenario: Reject registration with username below minimum length
    When the client sends POST /api/v1/auth/register with body { "username": "ab", "email": "alice@example.com", "password": "Str0ng#Pass1" }
    Then the response status code should be 400
    And the response body should contain a validation error for "username"

  Scenario: Reject registration with empty email
    When the client sends POST /api/v1/auth/register with body { "username": "alice", "email": "", "password": "Str0ng#Pass1" }
    Then the response status code should be 400
  ...
  Scenario: Reject registration with password below minimum length
    When the client sends POST /api/v1/auth/register with body { "username": "alice", "email": "alice@example.com", "password": "Short1!" }
    Then the response status code should be 400
    And the response body should contain a validation error for "password"
  ...
  Scenario: Reject registration with weak password — no lowercase letter
    When the client sends POST /api/v1/auth/register with body { "username": "alice", "email": "alice@example.com", "password": "STR0NG#PASS1" }
    Then the response status code should be 400
    And the response body should contain a validation error for "password"

  Scenario: Reject registration with weak password — no digit
    When the client sends POST /api/v1/auth/register with body { "username": "alice", "email": "alice@example.com", "password": "Str#ng#Pass!" }
    Then the response status code should be 400
    And the response body should contain a validation error for "password"

  Scenario: Reject registration with weak password — no special character
    When the client sends POST /api/v1/auth/register with body { "username": "alice", "email": "alice@example.com", "password": "Str0ngPass12" }
    Then the response status code should be 400
    And the response body should contain a validation error for "password"

  Scenario: Reject registration with invalid username format
    When the client sends POST /api/v1/auth/register with body { "username": "alice user!", "email": "alice@example.com", "password": "Str0ng#Pass1" }
    Then the response status code should be 400
    And the response body should contain a validation error for "username"

...
This application mirrors the same REST API contract as `demo-be-jasb` (Spring Boot) and
`demo-be-exph` (Elixir/Phoenix), providing an F#/Giraffe alternative implementation.
...
nx run demo-be-fsgi:test:quick

# Run unit tests only
nx run demo-be-fsgi:test:unit

# Run integration tests only (BDD with TickSpec + WebApplicationFactory)
nx run demo-be-fsgi:test:integration

# Lint with FSharpLint
nx lint demo-be-fsgi

# Type check (build with TreatWarningsAsErrors)
nx typecheck demo-be-fsgi
```
...
| ----------------- | -------------- | ------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| `DATABASE_URL`    | Yes (non-test) | —                                                | PostgreSQL connection string (e.g., `Host=localhost;Database=demo_be_fsgi;Username=demo_be_fsgi;Password=demo_be_fsgi`) |
...
`infra/dev/demo-be-fsgi/`. It will start PostgreSQL 17 and the F#/Giraffe application with
...
`demo-be-jasb` and `demo-be-exph`.

## See Also

- [demo-be-jasb](../demo-be-jasb/README.md) - Spring Boot implementation (same API)
- [demo-be-exph](../demo-be-exph/README.md) - Elixir/Phoenix implementation (same API)

...
Flutter mobile and web application for the Organic Lever Platform.

## Overview

This is a multi-platform Flutter application that provides a mobile and web client for the organic-lever-be backend API. The app is integrated into the Nx monorepo using vanilla Flutter (no plugins), following the explicit over implicit principle.
...
nx install organic-lever-app

# Run development server (web on port 3100)
nx dev organic-lever-app

# Run tests
nx test organic-lever-app

# Quick test (for pre-push hook)
nx test:quick organic-lever-app

# Lint code
nx lint organic-lever-app

# Build for web (production)
nx build:web organic-lever-app
```
...
The app connects to the organic-lever-be Spring Boot backend:
...
- Tags: `type:app`, `platform:flutter`, `domain:organic-lever`

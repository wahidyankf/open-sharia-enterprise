...
Each example is self-contained, runnable under `dotnet fsi`, and annotated to explain not just what the code does, but why each architectural decision was made. Start at the level that matches your current understanding and progress through the examples in order. Each FP example shares the same number as its OOP counterpart in the [in-oop-by-example](/en/learn/software-engineering/software-architecture/patterns-and-principles/in-oop-by-example) sibling tutorial, enabling cross-paradigm comparison.

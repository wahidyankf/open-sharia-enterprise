...
## Example 31: Basic Interface Definition and Implementation

An interface defines a contract (method signatures) without implementation. Classes implement interfaces using the `implements` keyword and must provide all method implementations.

**Code**:

```java
// INTERFACE DEFINITION
interface Printable {                     // => Interface: contract for printable objects
    void print();                         // => Abstract method: no body, implementors must define
}                                         // => All interface methods implicitly public abstract

// IMPLEMENTING CLASS
class Document implements Printable {     // => Explicit implementation via 'implements' keyword
                                          // => Must implement ALL interface methods
    private String content;               // => Private field: document data

    public Document(String content) {     // => Constructor: initializes content
        this.content = content;           // => Assigns parameter to field
    }

    @Override                             // => Annotation: confirms method from interface/parent
    public void print() {                 // => Implements Printable.print() method
        System.out.println("Document: " + content);
                                          // => Output: Document: [content value]
    }
}

// USAGE
Document doc = new Document("Invoice");   // => Creates Document instance
                                          // => doc.content is "Invoice"
doc.print();                              // => Calls print() method
                                          // => Output: Document: Invoice

// POLYMORPHISM via interface type
Printable item = new Document("Receipt");// => Declare as interface type
                                          // => Runtime type: Document, compile-time type: Printable
item.print();                             // => Calls Document.print() via interface reference
                                          // => Output: Document: Receipt
```

**Key Takeaway**: Interfaces define contracts through abstract methods. Classes explicitly declare implementation with `implements` and must provide all methods. Use `@Override` annotation to verify method correctness. Interface references enable polymorphism—treating different concrete types uniformly.

**Why It Matters**: Interfaces separate contract from implementation, enabling dependency injection (pass any `Printable` implementation), testing (mock interfaces easily), and flexibility (swap implementations without changing client code). This is foundational to Java's design patterns (Strategy, Factory, Dependency Injection) and frameworks like Spring. The explicit `implements` declaration makes architectural decisions visible—unlike duck typing in Python/JavaScript, Java's compiler verifies contracts at compile time, catching integration errors before runtime.

---

## Example 32: Multiple Interface Implementation

Java classes can implement multiple interfaces, gaining all their method contracts. This enables "mixin" style composition without the fragility of multiple inheritance.

**Code**:

```java
// FIRST INTERFACE
interface Readable {                      // => Interface: contract for readable objects
    String read();                        // => Abstract method: returns content as String
}

// SECOND INTERFACE
interface Writable {                      // => Interface: contract for writable objects
    void write(String content);           // => Abstract method: accepts content to write
}

// IMPLEMENTING MULTIPLE INTERFACES
class File implements Readable, Writable {// => Implements BOTH interfaces (comma-separated)
                                          // => Must provide ALL methods from both
    private String content = "";          // => Private field: file content storage

    @Override                             // => From Readable interface
    public String read() {                // => Implements Readable.read()
        return content;                   // => Returns current content
    }

    @Override                             // => From Writable interface
    public void write(String content) {   // => Implements Writable.write()
        this.content = content;           // => Updates content field
    }
}

// USAGE with different interface types
File file = new File();                   // => Creates File instance
                                          // => file.content is "" (empty string)
file.write("Hello");                      // => Calls write() method
                                          // => file.content is "Hello" (field updated)

Readable readableFile = file;             // => Upcast to Readable interface type
                                          // => Can only call Readable methods (read())
String content = readableFile.read();     // => Calls read() through Readable reference
                                          // => content is "Hello"
                                          // => Output: Hello

Writable writableFile = file;             // => Upcast to Writable interface type
                                          // => Can only call Writable methods (write())
writableFile.write("World");              // => Calls write() through Writable reference
                                          // => file.content is "World" (same object)
```

**Key Takeaway**: Classes can implement multiple interfaces by listing them comma-separated after `implements`. This provides multiple contracts without multiple inheritance problems. An object can be referenced through any of its interface types, restricting visible methods to that interface.

**Why It Matters**: Multiple interface implementation enables composition-based design without the "diamond problem" of multiple inheritance (C++). Each interface represents an independent capability (Readable, Writable, Comparable, Serializable)—classes choose which capabilities to provide. This design appears throughout Java APIs (ArrayList implements List, RandomAccess, Cloneable, Serializable—four independent contracts). Frameworks use this for cross-cutting concerns: Spring's @Transactional works on any interface, logging frameworks intercept method calls via interface proxies.

---

## Example 33: Interface Inheritance

Interfaces can extend other interfaces, creating hierarchies. Implementing classes must provide methods from all interfaces in the hierarchy.

```mermaid
%% Color Palette: Blue #0173B2, Orange #DE8F05, Teal #029E73, Purple #CC78BC, Brown #CA9161
graph TD
    Base["Vehicle<br/>(base interface)"] --> Car["Car<br/>(extends Vehicle)"]
    Base --> Boat["Boat<br/>(extends Vehicle)"]

    Car --> Sedan["Sedan<br/>(implements Car)"]
    Boat --> Sailboat["Sailboat<br/>(implements Boat)"]

    style Base fill:#0173B2,color:#fff
    style Car fill:#DE8F05,color:#fff
    style Boat fill:#DE8F05,color:#fff
    style Sedan fill:#029E73,color:#fff
    style Sailboat fill:#029E73,color:#fff
```
...
```java
// BASE INTERFACE
interface Vehicle {                       // => Base interface: common vehicle contract
    void start();                         // => Abstract method: start vehicle
    void stop();                          // => Abstract method: stop vehicle
}

// DERIVED INTERFACE
interface Car extends Vehicle {           // => Extends Vehicle interface (interface inheritance)
                                          // => Inherits start() and stop() methods
    void honk();                          // => Adds new method specific to cars
}                                         // => Total methods: start(), stop(), honk()

// IMPLEMENTING DERIVED INTERFACE
class Sedan implements Car {              // => Implements Car (includes Vehicle methods)
                                          // => Must implement ALL 3 methods
    @Override
    public void start() {                 // => From Vehicle (via Car)
        System.out.println("Sedan starting");
                                          // => Output: Sedan starting
    }

    @Override
    public void stop() {                  // => From Vehicle (via Car)
        System.out.println("Sedan stopping");
                                          // => Output: Sedan stopping
    }

    @Override
    public void honk() {                  // => From Car interface
        System.out.println("Beep beep!");
                                          // => Output: Beep beep!
    }
}

// USAGE via different interface types
Sedan sedan = new Sedan();                // => Creates Sedan instance
sedan.start();                            // => Calls start() method
                                          // => Output: Sedan starting
sedan.honk();                             // => Calls honk() method
                                          // => Output: Beep beep!

// POLYMORPHISM via base interface
Vehicle vehicle = sedan;                  // => Upcast to base Vehicle interface
                                          // => Can only call start(), stop() (not honk())
vehicle.start();                          // => Calls Sedan.start() via Vehicle reference
                                          // => Output: Sedan starting

// POLYMORPHISM via derived interface
Car car = sedan;                          // => Upcast to Car interface
                                          // => Can call start(), stop(), honk()
car.honk();                               // => Calls Sedan.honk() via Car reference
                                          // => Output: Beep beep!
```

**Key Takeaway**: Interfaces extend other interfaces using the `extends` keyword. Classes implementing derived interfaces must implement all methods from the entire hierarchy. This creates type hierarchies for polymorphism—objects can be referenced through any interface in their hierarchy.

**Why It Matters**: Interface inheritance enables progressive refinement of contracts. Java Collections Framework uses this extensively: `List extends Collection`, `SortedSet extends Set`—each level adds capabilities while maintaining compatibility with parent contracts. This design lets code accept broad interfaces (`Collection`) when order doesn't matter, or narrow interfaces (`SortedSet`) when ordering is required. Legacy code accepting `Collection` automatically works with new `List` implementations. This hierarchy design is why Java APIs rarely break backward compatibility—new interfaces extend old ones rather than replacing them.

---

## Example 34: Default Methods in Interfaces (Java 8+)

Default methods provide implementations inside interfaces. This enables interface evolution without breaking existing implementations.

**Code**:

```java
// INTERFACE with default method
interface Logger {                        // => Interface with abstract AND default methods
    ...
    default void logError(String message) {
        ...
        log("ERROR: " + message);         // => Calls abstract log() method
                                          // => Prepends "ERROR: " prefix
    }                                     // => Implementors get this for free

    default void logWarning(String message) {
                                          // => Another default method
        log("WARNING: " + message);       // => Calls abstract log() method
                                          // => Prepends "WARNING: " prefix
    }
}

// IMPLEMENTATION using defaults
class ConsoleLogger implements Logger {   // => Implements Logger interface
                                          // => Only needs to implement log() (not defaults)
    @Override
    public void log(String message) {     // => Implements required abstract method
        System.out.println(message);      // => Output: [message]
    }
    // logError() and logWarning() inherited from interface defaults
}

// IMPLEMENTATION overriding defaults
class CustomLogger implements Logger {    // => Another Logger implementation
    @Override
    public void log(String message) {     // => Implements required abstract method
    ...
    @Override
    public void logError(String message) {// => Overrides default implementation
        log("!!! ERROR !!! " + message);  // => Custom error format
    }
...
ConsoleLogger console = new ConsoleLogger();
                                          // => Uses default logError()
console.log("Info message");              // => Output: Info message
console.logError("File not found");       // => Uses default implementation
                                          // => Output: ERROR: File not found

CustomLogger custom = new CustomLogger(); // => Overrides logError()
custom.log("Info");                       // => Output: [LOG] Info
custom.logError("Crash");                 // => Uses overridden implementation
                                          // => Output: [LOG] !!! ERROR !!! Crash
custom.logWarning("Deprecated");          // => Uses default implementation
                                          // => Output: [LOG] WARNING: Deprecated
```

**Key Takeaway**: Default methods (Java 8+) provide implementations inside interfaces using the `default` keyword. Implementors inherit these methods automatically but can override them. This enables adding methods to interfaces without breaking existing implementations.

**Why It Matters**: Before Java 8, adding methods to interfaces broke ALL implementations—every class had to be updated. Default methods solved the "interface evolution problem," enabling the Streams API (`Collection.stream()`) to be added to existing collections without breaking millions of lines of code. This design pattern appears in modern Java APIs: `List.sort()`, `Map.forEach()`, `Comparator.reversed()` were all added via default methods. It's why Java 8 was revolutionary—functional programming features (streams, lambdas) could be retrofitted onto 20 years of existing APIs without breaking backward compatibility.

---

## Example 35: Static Methods in Interfaces (Java 8+)

Static methods in interfaces provide utility functions related to the interface. They're called on the interface name, not instances.

**Code**:

```java
// INTERFACE with static methods
interface MathUtils {                     // => Interface with static utility methods

    static int add(int a, int b) {        // => Static method: no instance needed
        return a + b;                     // => Returns sum
    }                                     // => Called via MathUtils.add(...)

    static int max(int a, int b) {        // => Another static method
        return (a > b) ? a : b;           // => Returns larger value using ternary
    }

    static boolean isEven(int n) {        // => Static predicate method
        return n % 2 == 0;                // => Returns true if n divisible by 2
    }
}

// INTERFACE with factory static methods
interface Product {                       // => Interface: product contract
    String getName();                     // => Abstract method: get product name
    double getPrice();                    // => Abstract method: get product price

    static Product create(String name, double price) {
                                          // => Static factory method
                                          // => Returns Product implementation
        return new BasicProduct(name, price);
                                          // => Creates BasicProduct instance
    }                                     // => Hides implementation details
}

// IMPLEMENTATION (package-private)
class BasicProduct implements Product {   // => Implementation class (not public)
    private String name;                  // => Private field: product name
    private double price;                 // => Private field: product price
...
int sum = MathUtils.add(5, 3);            // => Calls static method via interface name
                                          // => sum is 8
int maximum = MathUtils.max(10, 20);      // => Calls static method
                                          // => maximum is 20
boolean even = MathUtils.isEven(4);       // => Calls static predicate
                                          // => even is true

// USAGE of static factory
Product product = Product.create("Laptop", 999.99);
                                          // => Calls static factory method
                                          // => Returns BasicProduct instance (hidden)
                                          // => product (type: Product interface)
System.out.println(product.getName());    // => Output: Laptop
System.out.println(product.getPrice());   // => Output: 999.99
```

**Key Takeaway**: Interfaces can have static methods (Java 8+) called via the interface name, not instances. Use for utility functions (`MathUtils.add()`) or factory methods (`Product.create()`). Static methods cannot be overridden by implementations.

**Why It Matters**: Static interface methods enable bundling related utilities with interfaces—compare `Collections.sort(list)` (pre-Java 8 helper class) vs `Comparator.naturalOrder()` (Java 8+ interface static method). Factory methods hide implementation details (`Product.create()` returns interface type, not concrete class), enabling implementation swapping without changing client code. This pattern appears in modern Java APIs: `Stream.of(...)`, `Set.of(...)`, `List.copyOf(...)`—all static factory methods on interfaces. It consolidates utility methods where they belong (with the interface) rather than in separate helper classes (`Arrays`, `Collections`).

---

## Example 36: Functional Interfaces and Lambda Expressions

Functional interfaces have exactly one abstract method and can be implemented using lambda expressions (anonymous functions).
...
// TRADITIONAL implementation (anonymous class)
Calculator adder = new Calculator() {     // => Anonymous class implementation
    @Override
    public int calculate(int a, int b) {  // => Implements calculate method
        return a + b;                     // => Returns sum
    }
};
int sum = adder.calculate(5, 3);          // => Calls calculate()
                                          // => sum is 8

// LAMBDA implementation (concise)
Calculator multiplier = (a, b) -> a * b;  // => Lambda expression
                                          // => (parameters) -> expression
                                          // => Implements calculate(int, int)
int product = multiplier.calculate(5, 3); // => Calls calculate() via lambda
                                          // => product is 15

// LAMBDA with block body
Calculator divider = (a, b) -> {          // => Lambda with statement block
    if (b == 0) {                         // => Check for division by zero
        throw new IllegalArgumentException("Divide by zero");
    }
    return a / b;                         // => Returns quotient
};                                        // => Curly braces for multiple statements
int quotient = divider.calculate(10, 2);  // => Calls calculate()
                                          // => quotient is 5

// BUILT-IN functional interface: Predicate
import java.util.function.Predicate;      // => Standard functional interface
Predicate<Integer> isEven = n -> n % 2 == 0;
                                          // => Lambda: parameter n, returns boolean
                                          // => Implements test(Integer) method
boolean result = isEven.test(4);          // => Calls test() via lambda
                                          // => result is true

// BUILT-IN functional interface: Function
import java.util.function.Function;       // => Transforms input to output
Function<String, Integer> stringLength = s -> s.length();
                                          // => Lambda: String input, int output
                                          // => Implements apply(String) method
int length = stringLength.apply("Hello"); // => Calls apply() via lambda
                                          // => length is 5
```

**Key Takeaway**: Functional interfaces have exactly one abstract method (SAM - Single Abstract Method) and can be implemented using lambda expressions `(params) -> expression` or `(params) -> { statements; }`. Use `@FunctionalInterface` annotation to enforce contract. Java provides built-in functional interfaces in `java.util.function` package.

**Why It Matters**: Functional interfaces enabled Java to add functional programming features in Java 8 without breaking compatibility. Lambdas drastically reduce boilerplate compared to anonymous classes (4 lines vs 6+ lines for same logic). This design powers modern Java APIs: `list.forEach(item -> System.out.println(item))`, `stream.filter(n -> n > 0)`, `button.addActionListener(e -> handleClick(e))`. The `java.util.function` package provides 40+ standard interfaces (`Predicate`, `Function`, `Consumer`, `Supplier`) covering most use cases—use these before creating custom functional interfaces. This pattern eliminates entire categories of boilerplate code (Strategy pattern implementations, comparators, callbacks).

---

## Example 37: Comparable Interface - Natural Ordering

The `Comparable` interface defines natural ordering for a class. Implementing `compareTo()` enables sorting and comparison operations.
...
class Person implements Comparable<Person> {
                                          // => Comparable<Person>: compares Person objects
                                          // => Generic type parameter: Person
    private String name;                  // => Private field: person name
    private int age;                      // => Private field: person age
    ...
    @Override
    public int compareTo(Person other) {  // => Implements Comparable.compareTo()
                                          // => Returns: negative (this < other)
                                          // => Returns: 0 (this == other)
                                          // => Returns: positive (this > other)
        return Integer.compare(this.age, other.age);
                                          // => Compares by age (natural order)
                                          // => Uses Integer.compare() for safety
    }                                     // => Negative: this.age < other.age
                                          // => Zero: this.age == other.age
                                          // => Positive: this.age > other.age
...
// USAGE with sorting
import java.util.*;
Person p1 = new Person("Alice", 30);      // => Alice, 30 years old
Person p2 = new Person("Bob", 25);        // => Bob, 25 years old
Person p3 = new Person("Charlie", 35);    // => Charlie, 35 years old

List<Person> people = new ArrayList<>();  // => Creates mutable list
people.add(p1);                           // => Adds Alice
people.add(p2);                           // => Adds Bob
people.add(p3);                           // => Adds Charlie
                                          // => List order: [Alice(30), Bob(25), Charlie(35)]

Collections.sort(people);                 // => Sorts using compareTo() (natural order)
                                          // => After sort: [Bob(25), Alice(30), Charlie(35)]
                                          // => Sorted by age (ascending)

// DIRECT comparison
int comparison = p1.compareTo(p2);        // => Compares Alice(30) vs Bob(25)
                                          // => Returns positive (30 > 25)
                                          // => comparison > 0

// BINARY search (requires sorted list)
int index = Collections.binarySearch(people, new Person("Alice", 30));
                                          // => Searches for Alice(30) in sorted list
                                          // => Uses compareTo() for comparison
                                          // => index is 1 (second position, 0-based)
```

**Key Takeaway**: Implement `Comparable<T>` and define `compareTo(T other)` to establish natural ordering. Return negative (this < other), zero (equals), or positive (this > other). This enables sorting (`Collections.sort()`), binary search, and SortedSet/SortedMap usage.

**Why It Matters**: `Comparable` is Java's standard contract for natural ordering—appearing in 1000+ JDK classes (String, Integer, LocalDate, etc.). Implementing it unlocks automatic sorting in collections, efficient binary search, and TreeSet/TreeMap storage. The contract has strict requirements: must be consistent with `equals()` (if `a.compareTo(b) == 0` then `a.equals(b)` should be true), must be symmetric (`a.compareTo(b) == -b.compareTo(a)`), and transitive (if `a < b` and `b < c`, then `a < c`). Violating these contracts causes subtle bugs in sorted collections. Use `Integer.compare()`, `Double.compare()`, etc. instead of subtraction (`a - b`) to avoid integer overflow bugs.

---

## Example 38: Comparator Interface - Custom Ordering

The `Comparator` interface defines custom ordering separate from a class's natural order. Use for multiple sort criteria or sorting classes you don't control.
...
class Employee {                          // => No Comparable (no natural order)
    private String name;                  // => Employee name
    private int salary;                   // => Employee salary
    private int yearsOfService;           // => Years working at company
    ...
    public String toString() {
        return name + " ($" + salary + ", " + yearsOfService + " yrs)";
    }
}

// COMPARATOR 1: Sort by salary (descending)
Comparator<Employee> bySalary = new Comparator<Employee>() {
    ...
    public int compare(Employee e1, Employee e2) {
        ...
        return Integer.compare(e2.getSalary(), e1.getSalary());
                                          // => Reversed: e2 - e1 for descending
                                          // => Higher salary comes first
    }
};

// COMPARATOR 2: Sort by years (ascending) - lambda
Comparator<Employee> byYears = (e1, e2) ->
    Integer.compare(e1.getYearsOfService(), e2.getYearsOfService());
                                          // => Lambda comparator
                                          // => Compares years of service
                                          // => Ascending order (lower years first)

// COMPARATOR 3: Sort by name (alphabetical) - method reference
Comparator<Employee> byName = Comparator.comparing(Employee::getName);
                                          // => Comparator.comparing() factory method
                                          // => Employee::getName is method reference
                                          // => Compares by String natural order

// USAGE
List<Employee> employees = new ArrayList<>();
employees.add(new Employee("Alice", 80000, 5));
employees.add(new Employee("Bob", 60000, 10));
employees.add(new Employee("Charlie", 90000, 3));
                                          // => List: [Alice($80k,5y), Bob($60k,10y), Charlie($90k,3y)]

// SORT by salary (descending)
employees.sort(bySalary);                 // => Sorts using bySalary comparator
                                          // => Result: [Charlie($90k,3y), Alice($80k,5y), Bob($60k,10y)]

// SORT by years (ascending)
employees.sort(byYears);                  // => Sorts using byYears comparator
                                          // => Result: [Charlie(3y), Alice(5y), Bob(10y)]

// SORT by name
employees.sort(byName);                   // => Sorts using byName comparator
                                          // => Result: [Alice, Bob, Charlie] (alphabetical)

// CHAINING comparators (name, then years)
Comparator<Employee> byNameThenYears = byName.thenComparing(byYears);
                                          // => Combines comparators: primary=name, tiebreaker=years
employees.sort(byNameThenYears);          // => Sorts by name first, years on ties
```

**Key Takeaway**: `Comparator<T>` defines custom ordering via `compare(T o1, T o2)` method. Create using anonymous classes, lambdas, or `Comparator.comparing()` factory. Chain comparators with `thenComparing()`. Unlike `Comparable`, `Comparator` is external to the class being compared.

**Why It Matters**: `Comparator` solves critical limitations of `Comparable`: (1) sorting classes you don't control (String by length, not alphabetically), (2) multiple sort orders (Employee by salary vs years vs name), (3) different contexts (ascending vs descending). Modern Java provides powerful factory methods: `Comparator.comparing(Employee::getSalary).reversed()` creates descending salary comparator in one line. The `thenComparing()` chain enables multi-level sorting without complex logic. Java 8 streams use `Comparator` extensively: `stream.sorted(Comparator.comparing(Person::getAge).reversed())`. This pattern appears in all sorting scenarios beyond natural order.

---

## Example 39: Polymorphism - Method Overriding

Polymorphism allows treating different types uniformly through a common interface or parent class. Runtime method dispatch calls the actual object's method, not the reference type's method.

**Code**:

```java
// INTERFACE contract
interface Shape {                         // => Common interface for all shapes
    double area();                        // => Abstract method: calculate area
    String describe();                    // => Abstract method: describe shape
}

// IMPLEMENTATION 1: Circle
class Circle implements Shape {           // => Implements Shape interface
    private double radius;                // => Private field: circle radius

    public Circle(double radius) {        // => Constructor
        this.radius = radius;             // => Assigns radius
    }

    @Override
    public double area() {                // => Implements area() for circle
        return Math.PI * radius * radius; // => Formula: π * r²
    }                                     // => Returns area as double

    @Override
    public String describe() {            // => Implements describe() for circle
...
class Rectangle implements Shape {        // => Another Shape implementation
    ...
    @Override
    public double area() {                // => Implements area() for rectangle
        return width * height;            // => Formula: width × height
    }

    @Override
    public String describe() {            // => Implements describe() for rectangle
        return "Rectangle " + width + "x" + height;
    }
}

// POLYMORPHISM via interface reference
Shape shape1 = new Circle(5.0);           // => Reference type: Shape
                                          // => Actual object type: Circle
                                          // => Runtime type determines method called
Shape shape2 = new Rectangle(4.0, 6.0);   // => Reference type: Shape
                                          // => Actual object type: Rectangle

// POLYMORPHIC method calls
double area1 = shape1.area();             // => Calls Circle.area() (runtime dispatch)
                                          // => area1 is 78.54 (π * 25)
double area2 = shape2.area();             // => Calls Rectangle.area() (runtime dispatch)
                                          // => area2 is 24.0 (4 * 6)

// POLYMORPHIC processing
List<Shape> shapes = new ArrayList<>();   // => List of Shape references
shapes.add(new Circle(3.0));              // => Adds Circle (upcasted to Shape)
shapes.add(new Rectangle(5.0, 2.0));      // => Adds Rectangle (upcasted to Shape)
shapes.add(new Circle(7.0));              // => Adds another Circle

double totalArea = 0;                     // => Accumulator for total area
for (Shape shape : shapes) {              // => Iterates through polymorphic collection
                                          // => shape has type Shape (compile-time)
    double area = shape.area();           // => Calls actual object's area() method
                                          // => Runtime determines Circle vs Rectangle
    totalArea += area;                    // => Accumulates area
    System.out.println(shape.describe() + " has area " + area);
                                          // => Output varies by actual type
}                                         // => Output:
                                          // => Circle with radius 3.0 has area 28.27
                                          // => Rectangle 5.0x2.0 has area 10.0
                                          // => Circle with radius 7.0 has area 153.94
System.out.println("Total area: " + totalArea);
                                          // => Output: Total area: 192.21
```

**Key Takeaway**: Polymorphism enables treating different concrete types (Circle, Rectangle) uniformly through a common interface (Shape). Method calls are resolved at runtime based on actual object type, not reference type. This enables writing code that works with current and future implementations without modification.

**Why It Matters**: Polymorphism is the foundation of extensible design—write code accepting `Shape` interface, and it automatically works with Circle, Rectangle, Triangle, Polygon implementations without changes. This design appears throughout Java APIs: Collections accept `List` interface (works with ArrayList, LinkedList, CopyOnWriteArrayList), JDBC accepts `Connection` interface (works with MySQL, PostgreSQL, Oracle drivers). Frameworks like Spring rely on polymorphism for dependency injection—inject `PaymentProcessor` interface, swap implementations (Stripe, PayPal, Square) via configuration. This pattern eliminates massive if/else or switch statements checking types—each type knows its own behavior.

---

## Example 40: Marker Interfaces vs Annotations

Marker interfaces (empty interfaces) signal properties about a class. Modern Java prefers annotations for metadata.

**Code**:

```java
// LEGACY: Marker interface (empty interface)
interface Serializable {                  // => Marker interface: no methods
                                          // => Signals: class can be serialized
}                                         // => Used by Java serialization framework

class LegacyData implements Serializable {// => Implements marker interface
                                          // => Now eligible for serialization
    private String value;                 // => Private field

    public LegacyData(String value) {     // => Constructor
        this.value = value;               // => Assigns value
    }
}
...
@Retention(RetentionPolicy.RUNTIME)       // => Annotation retained at runtime
                                          // => Can be checked via reflection
@Target(ElementType.TYPE)                 // => Annotation applies to classes/interfaces
                                          // => Not applicable to methods/fields
@interface Persistent {                   // => Custom annotation definition
    String tableName() default "";        // => Annotation parameter with default
                                          // => Can specify table name
}

@Persistent(tableName = "users")          // => Annotates class with metadata
class User {                              // => User class (NOT implementing interface)
...
// CHECKING marker interface (runtime)
LegacyData data = new LegacyData("test"); // => Creates LegacyData instance
if (data instanceof Serializable) {       // => Checks if implements Serializable
                                          // => Runtime type check
    System.out.println("Object is serializable");
                                          // => Output: Object is serializable
}

// CHECKING annotation (reflection)
import java.lang.reflect.*;
User user = new User("alice", "alice@example.com");
                                          // => Creates User instance
Class<?> userClass = user.getClass();     // => Gets Class object (reflection)
                                          // => userClass is User.class
if (userClass.isAnnotationPresent(Persistent.class)) {
                                          // => Checks if class has @Persistent annotation
                                          // => Uses reflection API
    Persistent annotation = userClass.getAnnotation(Persistent.class);
                                          // => Retrieves annotation instance
    String tableName = annotation.tableName();
                                          // => Gets tableName parameter value
                                          // => tableName is "users"
...
**Key Takeaway**: Marker interfaces are empty interfaces signaling properties (Serializable, Cloneable). Modern Java prefers annotations (`@Persistent`, `@Override`) for metadata—they're more flexible (can have parameters), don't pollute type hierarchy, and clearly indicate compiler/framework usage. Use `instanceof` for marker interfaces, reflection for annotations.

**Why It Matters**: Marker interfaces were Java 1.0's only metadata mechanism—Serializable, Cloneable, RandomAccess signal framework behavior without methods. Their limitation: single purpose, can't carry data. Java 5 introduced annotations, revolutionizing metadata: JPA uses `@Entity`, Spring uses `@Component`, JUnit uses `@Test`—all carry parameters (`@Table(name = "users")`). Annotations don't pollute class hierarchy (User doesn't implement Persistent, just annotated). Frameworks introspect annotations at startup (Spring component scanning, Hibernate table mapping), eliminating XML configuration. Modern Java: use annotations for metadata, marker interfaces only for legacy compatibility (Serializable still used by RMI, ObjectOutputStream).

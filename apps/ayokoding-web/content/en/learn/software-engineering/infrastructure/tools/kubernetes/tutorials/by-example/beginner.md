...
kind: Pod
metadata:
  name: multi-container-pod # => Pod name: "multi-container-pod"
spec:
  containers:
    ...
    - name: busybox # => Sidecar container for data generation
      image: busybox:1.36
      command:
...
spec:
  initContainers:
    - name: init-setup # => First init container
      image: busybox:1.36
      command:
        - sh
        - -c
        - |
          ...
          echo "Initial content" > /work-dir/index.html
          sleep 2                         # => Simulates setup work
      volumeMounts:
        - name: workdir
          mountPath: /work-dir # => Writes to shared volume

    - name: init-permissions # => Second init container (runs after first)
      image: busybox:1.36
      command:
        - sh
        - -c
  ...
  containers:
    - name: nginx # => App container starts after both inits succeed
      image: nginx:1.24
      volumeMounts:
        - name: workdir
...
```yaml
apiVersion: apps/v1 # => Uses apps API group for workloads
kind: Deployment
metadata:
...
- Continue with [Intermediate Examples](/en/learn/software-engineering/infrastructure/tools/kubernetes/tutorials/by-example/intermediate) (Examples 29-57) for production patterns
- Or jump to [Advanced Examples](/en/learn/software-engineering/infrastructure/tools/kubernetes/tutorials/by-example/advanced) (Examples 58-85) for expert mastery

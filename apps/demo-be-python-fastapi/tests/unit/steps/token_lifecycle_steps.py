...
def alice_used_refresh_token(client: TestClient, registered_user: dict) -> dict:
    ...
    refresh_resp = client.post(
        "/api/v1/auth/refresh",
        json={"refresh_token": original_tokens["refreshToken"]},
    )
    assert refresh_resp.status_code == 200
    return {**original_tokens, "_original_refresh": original_tokens["refreshToken"]}
...
def alice_refresh(client: TestClient, alice_tokens: dict):  # type: ignore[no-untyped-def]
    return client.post(
        "/api/v1/auth/refresh",
        json={"refresh_token": alice_tokens["refreshToken"]},
    )

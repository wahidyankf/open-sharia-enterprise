...
**ayokoding-web** (Hextra): Bilingual ID/EN, 5 archetypes, educational content. Index titles match folder names (`lang` → `Lang`). Index navigation shows 3 layers deep (parent, children, grandchildren). **Index files in learn/belajar directories**: Navigation only (NO intro content - use separate overview.md). Author: site-level config only (NO in frontmatter except rants/celoteh for guests)

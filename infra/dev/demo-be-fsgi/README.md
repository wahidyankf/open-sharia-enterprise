...
Local development environment for `demo-be-fsgi`, the F#/Giraffe
alternative backend for the Demo Backend platform. Runs on the same port (8201) as the
Spring Boot backend (`demo-be-jasb`) and the Elixir/Phoenix backend (`demo-be-exph`) — the
...
| ------------------- | ------------------------------------------ | -------------------------------- |
| `POSTGRES_USER`     | `demo_be_fsgi`                             | PostgreSQL username              |
| `POSTGRES_PASSWORD` | `demo_be_fsgi`                             | PostgreSQL password              |

...
Local development environment for `demo-be-csas`, the C#/ASP.NET Core
alternative backend for the Demo Backend platform. Runs on the same port (8201) as the
Spring Boot backend (`demo-be-jasb`) and other alternative implementations — the
...
| ------------------- | ------------------------------------------ | -------------------------------- |
| `POSTGRES_USER`     | `demo_be_csas`                             | PostgreSQL username              |
| `POSTGRES_PASSWORD` | `demo_be_csas`                             | PostgreSQL password              |
